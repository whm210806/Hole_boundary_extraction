...
pcl::PointCloud<pcl::PointXYZ>::Ptr averagePointCloud(const std::string& bag_file, const std::string& topic_name) {
    ...
    writer.write<pcl::PointXYZ> ("/home/mike/sick_Ws/avg_cloud.pcd", *avg_cloud, false);
...
pcl::PointCloud<pcl::PointXYZ>::Ptr extract_ground(pcl::PointCloud<pcl::PointXYZ>::Ptr inputCloud){
    ...
    seg.setAxis(axis);
    seg.setEpsAngle(70 * (M_PI / 180.0f)); // plane can be within 10.0 degrees of X-Z plane
    ...
    // pass.filter(*inversed_cloud_plane);

    pcl::PassThrough<pcl::PointXYZ> pass;
    pass.setInputCloud(inversed_cloud_plane);      // 过滤平面点云
    pass.setFilterFieldName("y");          // 假设z轴为高度轴
    pass.setFilterLimits(-10, 0.093);        // 设置高度范围 (例如：0.1到1.0米)
    pass.setFilterLimitsNegative(false);   // 保留指定高度范围内的点
    pcl::PointCloud<pcl::PointXYZ>::Ptr filtered_cloud_plane(new pcl::PointCloud<pcl::PointXYZ>());
    pass.filter(*filtered_cloud_plane);  

    pcl::PCDWriter writer;
    writer.write<pcl::PointXYZ> ("/home/mike/sick_Ws/cluster_ground.pcd", *cloud_plane, false);
    writer.write<pcl::PointXYZ> ("/home/mike/sick_Ws/cluster_filtered.pcd", *filtered_cloud_plane, false);

    // pointClouds[0] = cloud_plane;
    // pointClouds[1] = inversed_cloud_plane;

    return filtered_cloud_plane;
}
...
int main(int argc, char** argv) {
    ...
    // 读取ROS bag文件
    std::string bag_file = "/home/mike/sick_Ws/src/test/bag/2d_3d_calibration_1.bag";  // 替换为你的ROS bag文件路径
